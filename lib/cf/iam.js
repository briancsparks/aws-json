...
var InstanceProfile = iam.InstanceProfile = function(namespace, service, cf, policyDocument) {
  ...
  var name = [toCapitalCase(namespace), toCapitalCase(service)].join('');
  self.output(name+"InstanceProfile", self, "InstanceProfile "+name);
  self.output(name+"InstanceProfileArn", self.getAtt("Arn"), "InstanceProfileArn "+name);
};
