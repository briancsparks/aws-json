...
var PublicSubnet = subnet.PublicSubnet = function(name, zoneLetter, cf, vpc) {
  ...
  self.ensureNatGateway = function() {
    var publicName  = name;
    var natEip      = eip.makeEip(publicName+'NatEip', cf);
    var natGw       = natGateway.makeNatGateway(publicName+'NatGateway', cf, self, natEip);
 ...
 *  PrivateSubnet
 */
var PrivateSubnet = subnet.PrivateSubnet = function(name, zoneLetter, cf, vpc, options_) {
  var self    = this;
  ...
  if (options.peeringConnection) {
    var peerRouteName = name+'PeerRoute';
    var peer          = cf.data.Resources[peerRouteName] = route.makeRoute(peerRouteName, cf, self.routeTable, {peeringConnection: options.peeringConnection});
    peer.destinationCidrBlock(options.peerCidrBlock);
  }
};

subnet.makePrivateSubnet = function(name, zoneLetter, cf, vpc, options) {
  return new PrivateSubnet(name, zoneLetter, cf, vpc, options);
};
