...
var PublicSubnet = subnet.PublicSubnet = function(name, zoneLetter, cf, vpc) {
  ...
  var rtName    = name+'PublicRouteTable';
  var routeName = name+'PublicRoute';

  self.publicRouteTable = cf.data.Resources[rtName]       = route.makeRouteTable(rtName, cf, vpc);

  BaseSubnet.call(this, name, zoneLetter, cf, vpc, self.publicRouteTable);

  self.isPublic         = true;
  self.publicRoute      = cf.data.Resources[routeName]    = route.makeRoute(routeName, cf, self.publicRouteTable, {internetGateway: vpc.internetGateway});
  ...
  self.ensureNatGateway = function() {
    ...
    var natRoute    = cf.data.Resources[publicName+'NatRoute']     = route.makeRoute(publicName+'natRoute', cf, self.publicRouteTable, {natGateway: natGw});
...
var PrivateSubnet = subnet.PrivateSubnet = function(name, zoneLetter, cf, vpc) {
  ...
  self.routeTable = route.makeRouteTable(rtName, cf, vpc);

  BaseSubnet.call(this, name, zoneLetter, cf, vpc, self.routeTable);

  cf.data.Resources[rtName] = route.makeRouteTable(rtName, cf, vpc);
